...
/* Curated source registry: exact source id → tier. Only a REGISTERED source
   may earn "trusted"; a trusted-looking name that is not registered is capped
   at "unverified", so trust cannot be spoofed by naming alone. In production
   this is per-tenant and provenance-backed; the default seeds the demo's
   genuinely-curated sources. */
export const SOURCE_REGISTRY = new Map([
  ["policy://governance/aup", "trusted"],
  ["sop://ops/refunds", "trusted"],
]);
export function registerSource(source, tier = "trusted") { SOURCE_REGISTRY.set(String(source), tier); return SOURCE_REGISTRY; }

export function sourceTrust(source, registry = SOURCE_REGISTRY) {
  const raw = String(source || "");
  const s = raw.toLowerCase();
  if (BLOCKED_SRC.some(r => r.test(s))) return { tier: "blocked", registered: false, ...SOURCE_TRUST_TIERS.blocked };
  const reg = registry ? (registry.get ? registry.get(raw) : registry[raw]) : null;
  if (reg && SOURCE_TRUST_TIERS[reg]) return { tier: reg, registered: true, ...SOURCE_TRUST_TIERS[reg] };
  // Unregistered: pattern heuristic only — "trusted" is unreachable without registration.
  if (INTERNAL_SRC.some(r => r.test(s))) return { tier: "internal", registered: false, ...SOURCE_TRUST_TIERS.internal };
  return { tier: "unverified", registered: false, ...SOURCE_TRUST_TIERS.unverified };
}
...
export function guardPassages(passages, opts = {}) {
  ...
  const registry = opts.registry || SOURCE_REGISTRY;
  const out = [];
  const dropped = [];
  for (const p of (passages || [])) {
    const trust = sourceTrust(p.source, registry);
