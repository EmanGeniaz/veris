...
/* Compute a full breaker row from a raw session { id, agent, signals[], started }.
   Pure: score, state, revoked capabilities and the accountable owner are all
   derived here from the signals — the ONE place the breaker's decision logic
   lives, shared by the seeded window and the live audit-chain view (BL-04). */
export function computeBreakerSession(r) {
  const agent = agentById(r.agent);
  const signals = r.signals || [];
  const score = Math.min(100, signals.reduce((s, k) => s + (SIGNALS[k]?.weight || 0), 0));
  ...
  const trigger = [...signals].sort((a, b) => (SIGNALS[b]?.weight || 0) - (SIGNALS[a]?.weight || 0))[0];
...
export function breakerSessions() {
  return RAW_SESSIONS.map(computeBreakerSession);
}

export function breakerStats(rows = breakerSessions()) {
