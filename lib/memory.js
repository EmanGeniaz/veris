...
export function rememberLive(entry, nowMs = Date.now()) {
  ...
  if (r.written) commitLive(r.item);
  return r;
}

/* Push an already-governed item into the in-process buffer (bounded). Used by
   the durable store as the no-DB fallback so it never re-classifies. */
export function commitLive(item) {
  LIVE.push(item);
  // Opportunistic retention: keep the buffer bounded so a warm instance can't grow without limit.
