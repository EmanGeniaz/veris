/* ── Veris Enforce · Runtime Guardrails ──────────────────────────────────
   ...
   Pure + deterministic (the caller passes the clock). Per-session state lives in
   a pluggable concurrency store (lib/concurrency-store): the default is the
   process-global store, but a shared or networked backing can be injected so the
   concurrency cap holds GLOBALLY across instances, not just per instance. */

import { defaultConcurrencyStore } from "./concurrency-store";
...
/* Per-session state comes from the pluggable concurrency store (default =
   process-global). Injecting a shared/networked store makes the cap global.
   key → { inFlight, recent:[action], calls:[ts], latencies:[ms] }. */
function state(key, store = defaultConcurrencyStore) { return store.get(key); }
...
/* Admit an agent tool call: loop → concurrency → rate, in that order. On allow,
   records the call and increments in-flight (balance with completeCall). State
   is read from `store` (default process-global) and committed back after
   mutation, so a shared/networked store enforces the cap globally. */
export function admitCall(key, action, nowMs = Date.now(), policy = RUNTIME_POLICY, store = defaultConcurrencyStore) {
  const s = state(key, store);
  ...
  if (loop.loop) { store.commit(key, s); return { decision: "loop", reason: `Loop detected (${loop.kind}: ${loop.action} ×${loop.count})`, rate, inFlight: s.inFlight, loop }; }
  ...
  if (rate >= policy.maxCallsPerWindow) { store.commit(key, s); return { decision: "throttle", reason: `Rate ${rate}/${policy.maxCallsPerWindow} in ${policy.rateWindowMs / 1000}s`, rate, inFlight: s.inFlight }; }
  s.inFlight++;
  s.calls.push(nowMs);
...
/* Complete an admitted call: decrement in-flight, record latency, flag SLA
   breach. */
export function completeCall(key, latencyMs, policy = RUNTIME_POLICY, store = defaultConcurrencyStore) {
  const s = state(key, store);
...
export function recordLatency(key, latencyMs, policy = RUNTIME_POLICY, store = defaultConcurrencyStore) {
  const s = state(key, store);
  s.latencies.push(latencyMs);
  if (s.latencies.length > 200) s.latencies.shift();
  store.commit(key, s);
