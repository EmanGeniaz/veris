...
export const GUARDRAIL_LAYERS = [
  ...
  },
  {
    ...
    engine: "runtime-guard.js (admission) + circuit-breaker.js (response)",
    controls: [
      ...
      { name: "Concurrency control", status: "partial", note: "A per-session in-flight cap throttles calls over the limit — enforced per instance; a global cap across instances needs shared state (Redis)." },
