...
export const GUARDRAIL_LAYERS = [
  ...
  },
  {
    ...
    controls: [
      ...
      { name: "Toxicity checks", status: "have", note: "classifyToxicity() scores every output across violence / self-harm / hate / harassment / sexual / profanity; a high-severity category withholds the response entirely." },
      { name: "Content moderation", status: "have", note: "moderateOutput() combines the toxicity verdict with the egress findings into one allow / flag / block decision run on every response." },
      { name: "Hallucination checks", status: "have", note: "hallucination.js scores faithfulness on every answer — each claim's figures and tokens are checked against the retrieved context — and escalates uncertain answers to a strict LLM judge that returns the unsupported claims; an unverifiable answer is flagged with a caution." },
    ],
  },
];
