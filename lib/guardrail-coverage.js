/* ── The 7 Guardrail Layers · honest coverage ────────────────────────────
   ...
   GenVeris actually enforces IN CODE, and grades each sub-control honestly:
...
export const GUARDRAIL_LAYERS = [
  {
    ...
    engine: "input-guard.js (ingressCheck) + policy-rules.ts — gateway ingress",
    controls: [
      ...
      { name: "Input validation", status: "have", note: "evaluateRules() blocks on any Block-action rule; classify() assigns the data class; the request body is shape-validated at ingress." },
      { name: "Content sanitization", status: "have", note: "sanitizeInput() strips zero-width, Unicode-Tag (E0000–E007F), bidi-override and control characters and neutralises <script> blocks — hidden-prompt injection can't ride in on invisible text." },
      { name: "Size & rate limits", status: "have", note: "Text is capped at ingress and a per-session request-rate limiter (40/min) throttles bursts." },
      { name: "Schema & MIME checks", status: "have", note: "Attachments must declare an allow-listed MIME type whose extension matches; mismatches are blocked at the door." },
      { name: "Malware & file scanning", status: "partial", note: "Deterministic checks — executable magic bytes (PE / ELF / Mach-O), dangerous and double extensions, and the EICAR test signature — block obvious malware, and scanAttachmentAsync() adds a pluggable external-AV seam (AV_SCAN_URL, fail-closed option). Still not a full AV engine or sandbox detonation unless a scanner is wired in." },
    ],
  },
  {
  ...
  },
  {
    ...
    engine: "retrieval-guard.js · guardPassages() wired into knowledge.ts retrieve()",
    controls: [
      ...
      { name: "Source filtering", status: "have", note: "Each passage's source is classified into a trust tier; blocked sources (paste sinks, untrusted dumps) are dropped before they can reach context." },
      { name: "Chunk validation", status: "have", note: "Junk / low-signal chunks are rejected and the DLP rulebook masks PII and blocks secret-bearing chunks — retrieval can't smuggle sensitive data into a prompt." },
      { name: "Metadata filtering", status: "have", note: "Passages are filtered by their metadata (source tier, freshness, data class) before entering the prompt." },
      { name: "Freshness checks", status: "have", note: "Age is computed from each document's createdAt; stale passages are flagged and down-weighted, expired ones can be dropped." },
      { name: "Trust scoring", status: "have", note: "Every source carries a trust score that weights ranking (relevance × trust × recency), and the \"trusted\" tier is unreachable by naming alone — only a source in the curated SOURCE_REGISTRY earns it, so trust can't be spoofed by a trusted-looking name. In production the registry is per-tenant and provenance-backed." },
    ],
  },
  {
    id: "memory", n: "04", name: "Memory Guardrails", tag: "Govern what the agent remembers",
    engine: "memory.js (governance) · memory-store.ts (durable Postgres path) — wired into the gateway pipeline",
    controls: [
      ...
      { name: "Memory retention rules", status: "have", note: "Each item is stamped with a class-based retention window at write (Confidential 24h, Internal 7d, Public 30d) and persisted to Postgres (AgentMemory) via memory-store.ts, partitioned by tenant+agent+session — durable across restarts and instances, with an in-process buffer as the no-DB fallback." },
      { name: "Memory expiry", status: "have", note: "Every item carries expiresAt; recallDurable() filters on it at query time so an expired item is never returned even before a sweep, and sweepDurable() deletes past-window rows for durable hygiene." },
    ],
  },
  {
    id: "runtime", n: "05", name: "Runtime Guardrails", tag: "Watch the session as it runs",
    engine: "runtime-guard.js (admission) + concurrency-store.js (pluggable cap state) + circuit-breaker.js (response)",
    controls: [
      { name: "Session monitoring", status: "have", note: "The circuit breaker watches each agent's risk signal per session and computes a 0-100 score; the runtime guard tracks each session's call stream, rate and latency." },
      { name: "Latency tracking", status: "have", note: "Every completed gateway call's latency is recorded and a call over the SLA (8s) is flagged as a breach — a real anomaly signal, not a seeded one." },
      { name: "Loop detection", status: "have", note: "admitCall() inspects the session's recent action stream for a repeated action or an A↔B cycle and halts the call before it spins." },
      { name: "Anomaly detection", status: "have", note: "Rate, loop and latency anomalies are computed from the live call stream at admission — the breaker's response ladder consumes these plus its own signals." },
      { name: "Concurrency control", status: "have", note: "A per-session in-flight cap throttles calls over the limit; the cap's state lives in a pluggable concurrency store (concurrency-store.js) so a shared or networked backing (injected Map, or a Redis/Postgres adapter) enforces one GLOBAL cap across instances — the process-global store is the per-instance fallback." },
      { name: "Fallback routing", status: "partial", note: "Suspend / halt route the session to a human and downscope keeps it read-only — graceful degradation, not model fallback routing." },
    ],
  },
  {
  ...
  },
  {
    ...
    engine: "output-guard.js (moderation) + policy-rules.ts validateResponse()",
    controls: [
      ...
      { name: "Toxicity checks", status: "have", note: "classifyToxicity() scores every output across violence / self-harm / hate / harassment / sexual / profanity as the always-on floor; classifyToxicityAsync() adds a model-grade moderation seam (MODERATION_URL) whose stricter verdict wins. A high-severity category withholds the response entirely." },
      { name: "Content moderation", status: "have", note: "moderateOutputAsync() combines the toxicity verdict (lexicon floor + optional model classifier) with the egress findings into one allow / flag / block decision run on every response; the classifier seam fails open to the floor." },
      { name: "Hallucination checks", status: "have", note: "hallucination.js scores faithfulness on every answer — each claim's figures and tokens are checked against the retrieved context (the always-on floor) — and runs a strict LLM judge that returns the unsupported claims; VZ_JUDGE_MODE controls how often the judge runs (auto = uncertain minority, always = every grounded answer, off). An unverifiable answer is flagged with a caution." },
    ],
  },
];
