...
export const ROLE_CENTERS = {
  ...
  ciso: {
    ...
    surfaces: [
      ...
      { id: "ciso_coverage", label: "Enforcement Coverage", sub: "Where control actually reaches — enforced inline vs observed vs shadow.", blocks: [
        { t: "text", eye: "Honest scope", h3: "Enforcement is a chokepoint, not action at a distance", body: "Veris decides what an agent does only where its traffic runs through the plane. This splits the estate into enforced (inline — block, mask, revoke), observed (out-of-band edge DLP) and shadow (detected, ungoverned), and computes real coverage — never a claim of control it does not have." } ] },
      { id: "ciso_paas", label: "Policy-as-a-Service", sub: "The policy engine as a callable service — one rulebook, every channel.", blocks: [
        { t: "text", eye: "Policy-as-a-Service", h3: "Policy the whole enterprise can call", body: "The same DLP + classification rulebook the Gateway enforces inline, exposed at one endpoint so a browser extension, CASB or CI pipeline enforces it on shadow-AI traffic too — allow · mask · block, every verdict signed into the evidence chain." } ] },
      ...
      { id: "ciso_egress", label: "Egress Policy", sub: "The containment guarantee — a successful injection can't reach the internet.", blocks: [
        { t: "text", eye: "Containment", h3: "Deny-by-default on the destination", body: "Least privilege stops an agent calling a tool it doesn't hold; egress policy stops the tools it does hold reaching a destination they shouldn't — an allow-list plus deny categories that close data exfiltration and SSRF against the cloud metadata service." } ] },
      { id: "ciso_hitl", label: "HITL Gates", sub: "Human oversight thresholds on high-impact actions (Art.14 / 22).", blocks: [
        { t: "text", eye: "Oversight", h3: "Where autonomy stops and a human decides", body: "High-impact actions are gated behind human approval with a threshold, so routine runs autonomously and the consequential routes to a named approver — mapped to EU AI Act Art.14 human oversight and Art.22 no-solely-automated-decision." } ] },
      { id: "ciso_breaker", label: "Circuit Breaker", sub: "Real-time capability revocation — downscope, suspend or halt an agent mid-session.", blocks: [
        { t: "text", eye: "Adaptive oversight", h3: "Continuous oversight, not just a fixed gate", body: "The breaker watches each agent's live risk signal during a session and revokes capability the moment it crosses a threshold — downscope to read-only, suspend to a human, or halt and revoke every token — before the agent reaches a pre-defined gate. Tokens are short-lived, so revocation is instant, and every trip is written to the Art.12 chain. The continuous, adaptive human oversight EU AI Act Art.14 requires." } ] },
      { id: "ciso_memory", label: "Memory Guardrails", sub: "Govern what an agent may remember — deny-by-default writes, class-based retention, partitioned recall.", blocks: [
        { t: "text", eye: "Memory", h3: "What an agent may remember and recall", body: "Every write runs the DLP rulebook — Restricted content is refused, PII is masked — and is stamped with a class-based retention window. Recall is partitioned by tenant + agent + session and re-checks class at read time, so a long-running agent never accumulates a private, ungoverned copy of sensitive data." } ] },
      { id: "ciso_input", label: "Input Guardrails", sub: "The first gate — invisible-character sanitization, attachment malware/MIME scan, size + rate limits.", blocks: [
        { t: "text", eye: "Input", h3: "Ingress is an injection surface", body: "Before classification the guard sanitises the text (stripping zero-width / Unicode-Tag / bidi / control characters so hidden-prompt injection can't ride in on invisible text), scans every attachment for executable magic bytes, dangerous or double extensions and MIME mismatches, and rate-limits the session." } ] },
      { id: "ciso_retrieval", label: "Retrieval Guardrails", sub: "What a passage must clear to ground an answer — source trust, chunk DLP, freshness, trust-weighted ranking.", blocks: [
        { t: "text", eye: "Retrieval", h3: "Retrieval is an injection surface", body: "Before a passage may ground an answer it must clear the guard: its source is classified into a trust tier (blocked sources dropped), the chunk is validated and DLP-cleaned (secret-bearing chunks blocked, PII masked), and survivors are re-ranked by relevance × source trust × recency so a trusted, fresh policy outranks a stale forum post." } ] },
      { id: "ciso_runtime", label: "Runtime Guardrails", sub: "How a session behaves in motion — loop detection, concurrency + rate caps, latency SLA.", blocks: [
        { t: "text", eye: "Runtime", h3: "The failure modes that only appear in motion", body: "At tool-call admission the guard inspects the session's recent action stream for a loop (a repeated action or an A↔B cycle) and halts it before it spins, throttles calls over the per-session concurrency and rate caps, and records every call's latency so one over the SLA is flagged as a real anomaly signal the circuit breaker can act on." } ] },
      { id: "ciso_output", label: "Output Guardrails", sub: "The last gate — toxicity classifier, groundedness heuristic on top of the secret/PII egress redactor.", blocks: [
        { t: "text", eye: "Output", h3: "The last gate before the user", body: "On top of the egress redactor that strips secrets and PII from the model's output, the guard runs a toxicity classifier (violence / self-harm / hate / harassment / sexual / profanity) and a groundedness heuristic that checks figures against the retrieved context. A high-severity category — or a secret that slipped the redactor — withholds the response entirely; lesser issues flag it." } ] },
      { id: "ciso_hallucination", label: "Hallucination Monitor", sub: "Faithfulness scoring on every answer — deterministic claim-grounding plus a strict LLM judge on uncertainty.", blocks: [
        { t: "text", eye: "Faithfulness", h3: "Make sure the model is grounded, not inventing", body: "Every answer is scored for faithfulness: each claim's figures and salient terms are checked against the retrieved context and overconfident absolutes are penalised. When the fast pass is uncertain, a strict LLM judge re-reads the answer against the context alone and returns the unsupported claims. An answer that can't be verified is scored down and flagged with a caution." } ] },
      { id: "ciso_guardlayers", label: "Guardrail Layers", sub: "The 7 guardrail layers every agentic stack needs — honest Enforced / Partial / Gap coverage, mapped to real engines.", blocks: [
        { t: "text", eye: "Guardrails", h3: "7 guardrail layers, honestly scored", body: "Input · Prompt · Retrieval · Memory · Runtime · Tool · Output — every sub-control scored against what GenVeris actually enforces in code, and mapped to the engine behind it. Not a green wall: some engines are strong, some run on seeded signals, some layers are not built yet." } ] },
  ...
  cgo: {
    ...
    surfaces: [
      ...
      { id: "cgo_breach", label: "Breach Notification", sub: "The regulatory clock — assess, decide, notify every authority whose window runs.", blocks: [
        { t: "text", eye: "Notify", h3: "One breach, every clock", body: "A confirmed personal-data breach or serious AI incident can run the GDPR 72-hour clock, India's DPDP and CERT-In 6-hour clocks and the EU AI Act serious-incident clock at once. The workflow assesses once, notifies against the tightest, and files one evidence pack." } ] },
      { id: "cgo_dataprov", label: "Data Provenance", sub: "One data-governance record per system — sources, lawful basis, IP, PII, integrity and a provenance hash.", blocks: [
        { t: "text", eye: "Govern the data", h3: "A model is only as governed as its data", body: "Each AI system carries a data-governance record across eight dimensions — source lineage, lawful basis, IP clearance, PII classification, quality, integrity, retention and a hashed provenance record — the one artifact that answers EU AI Act Art. 10, ISO 42001 A.7, the NIST data-poisoning defence, OWASP LLM03 and China's IP-clean-data duty at once." } ] },
      { id: "cgo_dsr", label: "Data Subject Rights", sub: "The people behind the data — access, rectification, erasure and consent honoured within each regime's statutory clock.", blocks: [
        { t: "text", eye: "Rights on the clock", h3: "Honour the right, prove it, in time", body: "For every system that processes personal data, one operating record shows how it answers access, rectification, erasure and consent — and a live queue runs each open request against the tightest window GDPR, the UAE PDPL, DIFC, India's DPDP and Brazil's LGPD impose. One control closes the data-subject-rights duties of all five at once." } ] },
      { id: "cgo_arabic", label: "Arabic Briefing (العربية)", sub: "Pilot: an Arabic + RTL governance briefing — proving localisation before estate-wide rollout.", blocks: [
        { t: "text", eye: "Localisation · pilot", h3: "Arabic + right-to-left", body: "A governance briefing that renders fully in Arabic with right-to-left layout via an on-surface language toggle — the pilot that proves the i18n pattern for the UAE customers before the rest of the platform is translated." } ] },
      { id: "cgo_aia", label: "Impact Assessments", sub: "One AIA / DPIA / FRIA per system, mapped to every regime that demands one.", blocks: [
        { t: "text", eye: "Assess once", h3: "One assessment, seven regimes", body: "A fundamental-rights assessment, a DPIA and an algorithmic impact assessment are the same nine questions asked by four regulators. Run them once per system and the EU AI Act Art. 27/9, GDPR Art. 35, ISO 42001, NIST RMF, Brazil and Korea impact-assessment obligations close together." } ] },
      { id: "cgo_carbon", label: "Environmental Footprint", sub: "Whole-life carbon per system + a GHG-Protocol-shaped disclosure (ISO/IEC TR 20226).", blocks: [
        { t: "text", eye: "Measure", h3: "Whole-life carbon, assessed and disclosed", body: "Inference is only the running cost. Each system carries a full-lifecycle impact assessment — training, inference, storage and retirement — and the estate rolls up into a carbon disclosure aligned to the GHG Protocol, ISO 14064-1 and CSRD ESRS E1, with an honest assurance level." } ] },
      ...
      { id: "cgo_templates", label: "Template Library", sub: "Framework template packs — ISO 42001/27001, NIST, EU AI Act — generated pre-filled from your control set.", blocks: [
        { t: "text", eye: "Template Library", h3: "Governance templates, ready to fill", body: "A repository of framework template packs. Each ships policy, Statement of Applicability, control checklist, impact assessment and RACI templates that generate pre-filled from your live control set and mint evidence on generation — never a blank document." } ] },
      ...
      { id: "cgo_workflows", label: "Agent Chain Permissions", sub: "Least privilege across agent chains — per step, per delegation, per composition.", blocks: [
        { t: "text", eye: "Autonomy", h3: "An agent can't escalate through a chain", body: "Least privilege holds at three altitudes (deny-by-default throughout): every step is re-checked against the agent's own capabilities; a sub-agent runs with min(orchestrator mandate, sub-agent grant) so a delegation can't widen data reach; and taint analysis across the whole chain catches emergent exfiltration paths that individually-allowed steps compose into." } ] },
      ...
      { id: "cgo_paas", label: "Policy-as-a-Service", sub: "The policy engine as a callable service — one rulebook, every channel.", blocks: [
        { t: "text", eye: "Policy-as-a-Service", h3: "Policy the whole enterprise can call", body: "The same DLP + classification rulebook the Gateway enforces inline, exposed at one endpoint so a browser extension, CASB or CI pipeline enforces it on shadow-AI traffic too — allow · mask · block, every verdict signed into the evidence chain." } ] },
      ...
      { id: "cgo_coverage", label: "Enforcement Coverage", sub: "Where control actually reaches — enforced inline vs observed vs shadow.", blocks: [
        { t: "text", eye: "Honest scope", h3: "How much of the estate is really on the plane", body: "Control reaches only the traffic routed through the plane. This splits the estate into enforced, observed and shadow and computes real coverage — the honest headline for a governance-only customer, not a claim of total control." } ] },
      ...
      { id: "cgo_hitl", label: "HITL Gates", sub: "Human-oversight thresholds — the Art.14 / 22 evidence a board reads.", blocks: [
        { t: "text", eye: "Oversight", h3: "Human oversight, proven", body: "Every high-impact AI action is gated behind a named approver with an SLA, thresholds keeping oversight meaningful rather than rubber-stamped — the EU AI Act Art.14 / Art.22 evidence a regulator and board ask for, with each decision logged." } ] },
      { id: "cgo_breaker", label: "Circuit Breaker", sub: "Real-time capability revocation — the adaptive half of Art.14 oversight.", blocks: [
        { t: "text", eye: "Adaptive oversight", h3: "Oversight that acts mid-session", body: "Beyond fixed gates, the breaker watches each agent's live risk signal and revokes capability the moment it crosses a threshold — downscope, suspend to a human, or halt and revoke every token — before the agent reaches a gate. Instant because tokens are short-lived, and every trip is written to the Art.12 chain with the accountable owner." } ] },
      { id: "cgo_memory", label: "Memory Guardrails", sub: "Govern what an agent may remember — deny-by-default writes, class-based retention, partitioned recall.", blocks: [
        { t: "text", eye: "Memory", h3: "What an agent may remember and recall", body: "Every write runs the DLP rulebook — Restricted content is refused, PII is masked — and is stamped with a class-based retention window. Recall is partitioned by tenant + agent + session and re-checks class at read time, so a long-running agent never accumulates a private, ungoverned copy of sensitive data." } ] },
      { id: "cgo_input", label: "Input Guardrails", sub: "The first gate — invisible-character sanitization, attachment malware/MIME scan, size + rate limits.", blocks: [
        { t: "text", eye: "Input", h3: "Ingress is an injection surface", body: "Before classification the guard sanitises the text (stripping zero-width / Unicode-Tag / bidi / control characters so hidden-prompt injection can't ride in on invisible text), scans every attachment for executable magic bytes, dangerous or double extensions and MIME mismatches, and rate-limits the session." } ] },
      { id: "cgo_retrieval", label: "Retrieval Guardrails", sub: "What a passage must clear to ground an answer — source trust, chunk DLP, freshness, trust-weighted ranking.", blocks: [
        { t: "text", eye: "Retrieval", h3: "Retrieval is an injection surface", body: "Before a passage may ground an answer it must clear the guard: its source is classified into a trust tier (blocked sources dropped), the chunk is validated and DLP-cleaned (secret-bearing chunks blocked, PII masked), and survivors are re-ranked by relevance × source trust × recency so a trusted, fresh policy outranks a stale forum post." } ] },
      { id: "cgo_runtime", label: "Runtime Guardrails", sub: "How a session behaves in motion — loop detection, concurrency + rate caps, latency SLA.", blocks: [
        { t: "text", eye: "Runtime", h3: "The failure modes that only appear in motion", body: "At tool-call admission the guard inspects the session's recent action stream for a loop (a repeated action or an A↔B cycle) and halts it before it spins, throttles calls over the per-session concurrency and rate caps, and records every call's latency so one over the SLA is flagged as a real anomaly signal the circuit breaker can act on." } ] },
      { id: "cgo_output", label: "Output Guardrails", sub: "The last gate — toxicity classifier, groundedness heuristic on top of the secret/PII egress redactor.", blocks: [
        { t: "text", eye: "Output", h3: "The last gate before the user", body: "On top of the egress redactor that strips secrets and PII from the model's output, the guard runs a toxicity classifier (violence / self-harm / hate / harassment / sexual / profanity) and a groundedness heuristic that checks figures against the retrieved context. A high-severity category — or a secret that slipped the redactor — withholds the response entirely; lesser issues flag it." } ] },
      { id: "cgo_hallucination", label: "Hallucination Monitor", sub: "Faithfulness scoring on every answer — deterministic claim-grounding plus a strict LLM judge on uncertainty.", blocks: [
        { t: "text", eye: "Faithfulness", h3: "Make sure the model is grounded, not inventing", body: "Every answer is scored for faithfulness: each claim's figures and salient terms are checked against the retrieved context and overconfident absolutes are penalised. When the fast pass is uncertain, a strict LLM judge re-reads the answer against the context alone and returns the unsupported claims. An answer that can't be verified is scored down and flagged with a caution." } ] },
      { id: "cgo_guardlayers", label: "Guardrail Layers", sub: "The 7 guardrail layers every agentic stack needs — honest Enforced / Partial / Gap coverage, mapped to real engines.", blocks: [
        { t: "text", eye: "Guardrails", h3: "7 guardrail layers, honestly scored", body: "Input · Prompt · Retrieval · Memory · Runtime · Tool · Output — every sub-control scored against what GenVeris actually enforces in code, and mapped to the engine behind it. The honest headline of what the platform's guardrails do and do not cover." } ] },
  ...
  cro: {
    ...
    attn: [
      ["Critical model risk — Credit Decision", "Residual High after treatment; Art.22 exposure needs board note.", "Open register", "crit", "cro_register"],
...
/* ══════════ People-manager = employee + Team View ══════════
   Every leader (manager, director, VP, BU head …) is also an employee, so a
   people-manager's workspace IS the employee workspace — same home, same
   personal surfaces — with two additions: a "Team View" surface for the people
   they lead, and a team band in the focus of their main page. Built by
   extending the employee center so the individual experience stays one source
   of truth. */
const MGR_TEAM_VIEW = { id: "mgr_team", label: "Team View", sub: "Your team's AI at a glance — adoption, value, risk, and what needs your sign-off. Aggregates only, never private prompts.", blocks: [
  { t: "kpis", items: [["Team adoption", "64%", "good", "14 of 22 active"], ["Time saved", "118h", "good", "team · month"], ["Approvals", "5", "warn", "waiting on you"], ["Team compliance", "86%", "warn", "3 outstanding"], ["Guardrail saves", "9", "gold", "team · month"], ["Usage cost", "$1.2K", "info", "this month"]] },
  ...
  { t: "actions", eye: "Approvals queue", h3: "5 waiting on you", items: [
    ...
    [2, "info", "Policy exception — extended data retention", "A 30-day retention waiver for a summarisation flow, above the standard 90-day rule. Routes to the CDPO office if approved.", "Approve", "Defer",
      { by: "S. Kim (Specialist) — requested on behalf of the Resolution Copilot pilot", why: "The flow needs 120-day retention to evaluate summary quality over a full quarter, beyond the standard 90-day rule.", next: "Approve → your sign-off routes to the CDPO office for final ratification. Defer → holds for more detail.", wait: "Pending 2 days · then CDPO ratification", ref: "Initiative: Resolution Copilot · exception EXC-2026-014" }],
    [3, "warn", "Tool access — model sandbox (2 analysts)", "Two analysts requested sandbox access to prototype a workflow; access is time-boxed and logged.", "Grant", "Decline",
      { by: "L. Haddad and J. Okafor (Analysts) — joint request to prototype an automation", why: "Sandbox access lets them test a workflow against real models without touching production data; it's time-boxed and fully logged.", next: "Grant → 30-day sandbox access for both, auto-expiring with an audit trail. Decline → returns with your reason.", wait: "Pending 1 day · with you", ref: "Tool: Model sandbox · time-boxed 30 days · 2 members" }],
    [4, "warn", "New project — Support Insights Copilot (J. Okafor)", "A proposed AI initiative from your team; approving files it into AI Central intake and creates the cross-functional facets.", "Approve", "Decline",
      { by: "J. Okafor (Analyst) — self-initiated proposal 2 days ago", why: "A new AI initiative can't be built until a manager owns the risk; your approval opens a governed workspace with an AI-risk assessment.", next: "Approve → creates the initiative and its Legal/Risk/Security facets in AI Central. Decline → returns to J. Okafor with a reason.", wait: "Pending 2 days · with you", ref: "Would become AI initiative · Customer Ops" }],
    [5, "info", "Scale gate — Support Automation", "The pilot met its adoption and risk bar; approve to move it into the scaling wave with a governed decision record.", "Approve", "Hold",
      { by: "Gate engine — auto-raised when Support Automation cleared its pilot exit criteria", why: "The pilot hit its adoption (9/22) and residual-risk (Low) thresholds; moving to scale needs a manager's governed decision.", next: "Approve → advances it to the scaling wave with a dated decision record and notifies AI Central. Hold → keeps it in pilot.", wait: "Awaiting your decision · lead L. Haddad", ref: "Initiative: Support Automation · pilot → scale gate" }]] },
...
ROLE_CENTERS.manager = {
  ...ROLE_CENTERS.employee,
  thesis: "Your own AI workspace — plus a Team View, because you also lead people.",
  greet: "You saved 6.2 hours this week",
  sub: "2 tasks due and 2 sessions in progress — and, as a people-leader, 5 approvals and 2 enablement nudges wait in your Team View.",
  hero: ["82", "AI productivity", "6.2h saved this week · +8"],
  attn: [
    ...ROLE_CENTERS.employee.attn.slice(0, 2),
    ["5 approvals from your team", "Prompt sign-offs, a policy exception and access requests need your decision.", "Open Team View", "warn", "mgr_team"],
    ["2 members need enablement", "Two people are below the active-use threshold — a nudge would help.", "Open Team View", "info", "mgr_team"],
  ],
  kpis: [
    ...ROLE_CENTERS.employee.kpis.slice(0, 4),
    ["Team adoption", "64%", "good", "14/22 · Team View", LN("your team active on AI ÷ team size", [["Active this week", "14", "of 22"], ["Below threshold", "2", "need enablement"]], "Your team's adoption — open Team View for the roster and enablement plan.")],
    ["Approvals", "5", "warn", "waiting on you · Team View", LN("governance decisions in your queue", [["Prompt sign-offs", "2", "team"], ["Policy exception", "1", "→ CDPO"], ["Access + scale", "2", "team"]], "Five decisions await your sign-off — open Team View for each with its context.")],
  ],
  surfaces: [...ROLE_CENTERS.employee.surfaces, MGR_TEAM_VIEW],
};
