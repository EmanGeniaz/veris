/* ── GenVeris · Hallucination / Faithfulness engine ──────────────────────
   ...
   2. LLM judge (optional second pass): when the deterministic check is not
      clearly grounded, a strict verifier model call re-checks the answer
      against the context and returns the unsupported claims. This is the
      accurate-but-costlier tier, so it runs only on the uncertain minority.
...
/* When to escalate to the LLM judge: context exists and the fast check isn't
   clearly grounded. */
