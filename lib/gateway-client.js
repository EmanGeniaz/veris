...
export async function askGateway(prompt, tenant = "demo") {
  try {
    ...
    // An honest upstream/timeout/validation error (d.error, or a non-2xx status)
    // is surfaced as "unavailable" — the UI degrades gracefully and never
    // fabricates a result, while the server still returns the real status/reason.
    if (!d || d.enabled === false || d.error || !res.ok) return { enabled: false, text: "" };
