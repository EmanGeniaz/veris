...
import { DataSubjectRights } from "./platform/data-subject-rights";
...
const ROLE_PAGE_OVERRIDE={emp_assistant:"workbench",mgr_assistant:"workbench",emp_usage:"aiusage",mgr_usage:"aiusage",emp_learning:"academy",
  ...
  cgo_forum:"govforum",cgo_incidents:"incidents",cgo_breach:"breach",cgo_aia:"aia",cgo_dataprov:"dataprov",cgo_dsr:"dsr",cgo_carbon:"carbon",cgo_arabic:"arabicpilot",cgo_crosswalk:"crosswalk",cgo_redlines:"redlines",cgo_gpai:"gpai",
...
const PageRoadmap=dynamic(()=>import("./platform/dashboard").then(m=>m.PageRoadmap),{ssr:false,loading:vzLoading});
const PageDecisions=dynamic(()=>import("./platform/dashboard").then(m=>m.PageDecisions),{ssr:false,loading:vzLoading});
const PagePlaybook=dynamic(()=>import("./platform/playbook").then(m=>m.PagePlaybook),{ssr:false,loading:vzLoading});
const PageComplianceStandards=dynamic(()=>import("./platform/compliance").then(m=>m.PageComplianceStandards),{ssr:false,loading:vzLoading});
const PageRiskCenter=dynamic(()=>import("./platform/riskcenter").then(m=>m.PageRiskCenter),{ssr:false,loading:vzLoading});
const PageReports=dynamic(()=>import("./platform/reports").then(m=>m.PageReports),{ssr:false,loading:vzLoading});
const PageGovernanceAcademy=dynamic(()=>import("./platform/academy").then(m=>m.PageGovernanceAcademy),{ssr:false,loading:vzLoading});
const PageModelRegistry=dynamic(()=>import("./platform/aicentral").then(m=>m.PageModelRegistry),{ssr:false,loading:vzLoading});
const PageMaturityRadar=dynamic(()=>import("./platform/aicentral").then(m=>m.PageMaturityRadar),{ssr:false,loading:vzLoading});
      ...
      <nav className="vz-side-nav" style={{flex:1,padding:"10px 9px",overflowY:"auto"}}>
        {!acOnly&&roleNavSections.map(section=>{
          const items=section.items.map(id=>navById[id]).filter(Boolean).map(it=>{
            if(it.id==="home"){
              if(role==="employee"||role==="manager")return {...it,label:"My AI Workspace"};
  ...
  return <LangContext.Provider value={lang}><div dir={rtl?"rtl":"ltr"} lang={lang} style={{display:"flex",minHeight:"100vh",background:T.bg}}>
    ...
    <div style={{marginLeft:isMobile?0:(rtl?0:SIDEBAR_W),marginRight:isMobile?0:(rtl?SIDEBAR_W:0),flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
      ...
      <div data-testid="vz-main" style={{flex:1,padding:"20px 24px 60px",maxWidth:1320,width:"100%",margin:"0 auto"}}>
        ...
        {showSeededData&&ROLE_PAGE_OVERRIDE[tab]==="dsr"&&<DataSubjectRights role={role} showToast={showToast}/>}
        ...
        {showSeededData&&ROLE_PAGE_OVERRIDE[tab]==="redlines"&&<ProhibitedPractices role={role} showToast={showToast}/>}
        {showSeededData&&ROLE_PAGE_OVERRIDE[tab]==="gpai"&&<GpaiExposure role={role} showToast={showToast}/>}
        {showSeededData&&ROLE_PAGE_OVERRIDE[tab]==="gapclosure"&&<GapClosure role={role} showToast={showToast}/>}
        {showSeededData&&ROLE_PAGE_OVERRIDE[tab]==="jurisdictions"&&<JurisdictionAtlas role={role} showToast={showToast}/>}
