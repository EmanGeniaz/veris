...
| High | Verified | Glyph/Text | `components/GenVerisPlatform.jsx:5`, `:239`, `:1332` | Source contains mojibake/glyph corruption |
| High | Verified | Auth/RBAC | `BrandEntryShell`, `GenVeris` state | Login is client-side demo entry; credentials are not validated |
| High | Verified | Architecture | `components/GenVerisPlatform.jsx`, 4k+ lines | Main app is monolithic, hard to regress safely |
| High | Verified | Routing | `app/page.tsx`; hash parsing in `GenVeris` | Modules are local/hash tabs, not route-backed screens |
